 ...
 * `table-created`: `openTable` returns the table itself and no events, so
 * such a variant would be a case every downstream switch has to handle for a
 * thing that can never reach it.
 */
