...
### What this plan hands it, decided and written down

Five things were settled during execution and are owed to the next plan rather than to this one. They are here because the working notes that recorded them do not survive, and each is a decision someone would otherwise have to make again from nothing.

1. **The leave control.** `leave` exists on the wire, in the validator, and in `apps/server/src/session.ts`; `removeParticipant` frees the seat, migrates the baton, and `#empty` returns the table to what a screen opens. **No client sends it** — the phone has no button. The machine is built and unreachable, which also means a table cannot be reset without restarting the server, and that made the first television run harder than it needed to be. Adding the button is small and closes the mechanism.
2. **Extract the phone's screens first, not last.** `apps/phone/src/App.tsx` is one component holding seven screen branches. The whole-branch review carried that on the explicit condition that Plan 2b extract `ChoosingScreen`, `PreviewScreen` and `SeatingScreen` as its **first** act — this plan adds the match screens to the same file, and extracting after knowing their shape means doing it twice.
3. **Finish the round marker.** `packages/core` refuses a stale round and is tested for it, and `apps/server` passes it through from `hello`. Nothing advances `Table.round`, and the phone never sends one, so no device can produce a `stale-round` refusal today. Both missing halves belong here: the bump comes with "Clear seats" and "Change game", and the phone must persist and send its round alongside its token.
4. **One surviving edge of the crash class.** `apps/server/src/session.ts`'s disconnect path is not covered by the transport's `#deliver` guard, so a translation throw during a disconnect could still unwind into engine.io the way an uncaught message-handling throw once did. Unreachable by crafted content — a disconnect carries no payload — and deliberately left rather than widened into a catch-all.
5. **The television run for this plan is outstanding.** `docs/tv-smoke-test.md` steps 17-21 are written and unrun. Two defects were already found on real hardware before those steps were reached, both invisible to every automated test: the seating screen built its table surface inside the QR branch, so filling the last seat removed the table itself; and all four covers carried a `viewBox` with no `width`/`height`, so they served correctly and drew nothing. Both are fixed and guarded. The rest of the checklist has not been run.
